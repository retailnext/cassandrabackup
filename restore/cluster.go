...
func RestoreCluster(ctx context.Context, cfg config.Config) error {
	lgr := zap.S()
	...
	identities := nodeIdentitiesForCluster(ctx, cfg, clusterCmdCluster, clusterCmdHostnamePattern)
	...
	for _, hostIdentity := range identities {
		...
		nodePlan, err := plan.Create(ctx, cfg, hostIdentity, unixtime.Seconds(*clusterCmdNotBefore), unixtime.Seconds(*clusterCmdNotAfter))
		if err != nil {
			return err
		}
	...
	w := newWorker(cfg, *clusterCmdTargetDirectory, false)
	return w.restoreFiles(ctx, files)
}

func nodeIdentitiesForCluster(ctx context.Context, cfg config.Config, cluster, prefix *string) []manifests.NodeIdentity {
	expr := regexp.MustCompile("^" + regexp.QuoteMeta(*prefix) + ".+$")
	return nodeidentity.ForRestoreMatchingRegexp(ctx, cfg, *cluster, expr)
}
