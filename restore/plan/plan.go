...
func Create(ctx context.Context, config *config.Config, identity manifests.NodeIdentity, startAfter, notAfter unixtime.Seconds) (NodePlan, error) {
	lgr := zap.S().With("identity", identity)

	nodeManifests, err := getManifests(ctx, config, identity, startAfter, notAfter)
	if err != nil {
...
func getManifests(ctx context.Context, config *config.Config, identity manifests.NodeIdentity, startAfter, notAfter unixtime.Seconds) ([]manifests.Manifest, error) {
	client := bucket.OpenShared(config)
