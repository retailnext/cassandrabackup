...
func ForRestore(ctx context.Context, config *config.Config, overrideCluster, overrideHostname, overridePattern *string) manifests.NodeIdentity {
	...
	filtered := ForRestoreMatchingRegexp(ctx, config, result.Cluster, expr)
	if len(filtered) != 1 {
...
func ForRestoreMatchingRegexp(ctx context.Context, config *config.Config, cluster string, expr *regexp.Regexp) []manifests.NodeIdentity {
